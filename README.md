...
- **快速哈希验证**：SHA-256（文件头尾各 64KB + 文件大小），~128KB I/O 成本，用于传输后完整性校验
...
|------|------|------|
| 相对路径匹配 + 大小相同 | 一致 | 跳过 |
| 相对路径匹配 + 大小不同 | 差异 | 源覆盖目的 |
| 仅在源端 | 新文件 | 复制到目的 |
| 仅在目的端 | 多余 | 从目的删除 |

比对阶段不涉及哈希——哈希验证在传输阶段由 `transfer_with_verify()` 和 `safe_delete_remote()` 负责。
...
用于传输后完整性验证（`transfer_with_verify`）和 Phone 端安全删除前的备份校验（`safe_delete_remote`）。
比对阶段不进行哈希计算——因为 `file_size` 参与哈希，不同大小的文件哈希必然不同，比对时无必要。
