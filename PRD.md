...
| 2 | 选择 Phone 时自动检测 ADB 设备并预填 `/sdcard/Music/`，未检测到时给出配置指引 |
...
| 7 | 相对路径匹配 + 文件大小不同 → 快速哈希确认差异 → 标记为源覆盖目的 |
| 8 | 比对过程中可随时取消 |
...
| 确认路径 | 相对路径匹配 + 大小不同 | 计算快速哈希确认 → 需要覆盖 | ~128KB/文件 |
...
musicsync/
  ...
  core/                — 核心逻辑层
    ...
    sync_engine.py     — scan / hash / compare 纯函数管道
...
仅对 `src.size != dst.size` 的文件对按需计算哈希（两端各自计算，供比对确认和执行阶段校验使用）：
...
| 比对 | 大小相同→跳过、大小不同→哈希确认→覆盖、仅源端→复制、仅目的端→删除 |
| 传输校验 | 传输 + 哈希比对 + 失败重试 + 二次失败标记 |
