...
- 相对路径匹配 + 大小相同 → 跳过（已同步）
- 相对路径匹配 + 大小不同 → 源覆盖目的（比对阶段不哈希；quick_hash 含 file_size 使哈希比对冗余）
- 仅在源端 → 复制到目的
- 仅在目的端 → 从目的删除

哈希验证在 **execute 阶段** 才进行：
- `transfer_with_verify()` — 每次 copy/overwrite 后用 quick_hash 比对源端 vs 目的端
...
**用途**：仅在 execute 阶段（传输后完整性验证 + 删除前备份验证），比对阶段不再使用。

### 经验库

`MEMORY.md` 是项目经验索引文件，每条一行标题+摘要。详细记录在 `memory/*.md`。
修改比对/哈希/执行逻辑时，**必须先读取 MEMORY.md** 检查相关教训。
